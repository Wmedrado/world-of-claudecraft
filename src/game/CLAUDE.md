...
- **SFX assets follow one standard.** Every clip under `public/audio/sfx/` is MP3
  44.1 kHz, 192 kbps, normalized loudness, and mono, EXCEPT ambience loops
  (`amb_*`, flagged `stereo: true` in the catalog), which stay stereo. Positional
  `playAt` downmixes to mono through the panner and `playUi` sums to the mono
  master, so a second channel is decoded and discarded. The full standard (format,
  loudness, channel policy per playback path, bitrate ceiling, naming) is
